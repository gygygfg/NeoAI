...
function M.send_stream_request(params, on_chunk, on_complete, on_error)
  ...
  -- 性能优化：使用时间戳替代每次创建/销毁定时器
  -- handle_stdout 只需赋值 last_data_time，无需任何 timer 操作
  -- 持久定时器每 15s 检查一次，开销可忽略
  local last_data_time = 0
  local idle_timer = nil

  local function start_idle_timer_persistent()
    ...
    if r then
      r.idle_timer = idle_timer
    end
    -- 每 15s 检查一次（IDLE_TIMEOUT_MS=30s，两倍余地）
    idle_timer:start(
      IDLE_TIMEOUT_MS / 2,
      IDLE_TIMEOUT_MS / 2,
      vim.schedule_wrap(function()
        ...
        local elapsed = vim.uv.now() - last_data_time
  ...
  -- 启动持久空闲超时定时器（每 15s 检测一次，30s 无数据触发）
  start_idle_timer_persistent()
  -- 更新去重缓存
  if generation_id then
  ...
  local function handle_stdout(data_lines)
    ...
    -- 收到数据时更新时间戳（持久定时器检测空闲）
    last_data_time = vim.uv.now()
    -- 计算本次回调的数据总大小
    local lines_size = 0
