...
function M.read_file(path)
  ...
  if not content then
    ...
    -- 返回读取错误
    return nil, "读取文件内容失败: " .. (read_err or "未知错误")
  end
  ...
  if file then
    ...
    if not close_ok then
      return nil, "关闭文件失败: " .. (close_err or "未知错误")
    end
  end
...
--- @return boolean|nil, string|nil 成功时返回true，错误时返回nil和错误信息
...
--- 读取行
--- @param path string 文件路径
--- @return table|nil, string|nil 行列表，错误时返回nil和错误信息
...
function M.copy_file(src, dst)
  ...
  return M.write_file(dst, content, false)
end
...
--- @return table|nil, string|nil 文件列表，错误时返回nil和错误信息
