...
tests.suite("session", function(_, it)
  ...
  it("fork 分支：复制消息 + 父链", function(t)
    local session = require("NeoAI.core.session.session")
    local s = session.create({ model = "m1", provider = "p1" })
    ...
    t.eq("m1", child.model)
    t.eq("p1", child.provider, "fork 继承 provider")
