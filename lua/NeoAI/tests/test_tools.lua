...
tests.suite("tools", function(_, it)
  ...
  it("packer 分组", function(t)
    ...
    t.ok(grouped.lsp)
    t.eq(1, #grouped.file)
    t.eq(10, #packer.get_all_packs())
    t.not_nil(packer.pack_display_name("file"))
    t.eq("网页抓取", packer.pack_display_name("web"))
  end)
  ...
  it("list_files 单层列出非空，空目录给出明确文案", function(t)
    local registry = require("NeoAI.tools.registry")
    registry.reset()
    local config_store = require("NeoAI.kernel.config_store")
    config_store.load({ tools = { approval = { mode = "auto_allow" } } })
    ...
    executor.execute("list_files", { path = dir, description = "列目录" }, {}):then_(function(r)
      out = r; done = true
    end, function(e) out = "ERR:" .. tostring(e); done = true end)
    t.true_(vim.wait(2000, function() return done end), "list_files 应完成")
    t.matches("a%.txt", out, "应列出文件")
    t.matches("sub/", out, "应列出子目录并带斜杠")

    local empty = vim.fn.tempname()
    vim.fn.mkdir(empty, "p")
    done = false
    executor.execute("list_files", { path = empty, description = "列空目录" }, {}):then_(function(r)
      out = r; done = true
    end, function(e) out = "ERR:" .. tostring(e); done = true end)
    t.true_(vim.wait(2000, function() return done end), "空目录 list_files 应完成")
    t.eq("(空目录)", out, "空目录应返回明确文案而非空串")
    vim.fn.delete(dir, "rf")
  ...
  it("沙箱新建目录树对 list_files/file_exists/search_files 一致可见（不逃逸）", function(t)
    ...
    local config_store = require("NeoAI.kernel.config_store")
    local saved = config_store.get_all()
    -- /tmp、/var/tmp 现为每会话私有 tmpfs（不冻结候选），故改用 /root 作为显式可写根，
    -- 并把 cwd 移到中立目录以避免 /root 遮蔽目录作用域。
    config_store.load({ tools = { approval = { mode = "async" }, sandbox = {
      mode = "dry_run", review = { enabled = true }, process_roots = { "/root" },
    } } })
    local executor = require("NeoAI.tools.executor")
    registry.register_many(require("NeoAI.tools.builtin.file_ops").get_tools())
    registry.register_many(require("NeoAI.tools.builtin.shell").get_tools())
    local sandbox = require("NeoAI.sandbox")
    sandbox.reset()
    local fs = require("NeoAI.utils.fs")
    local prev_cwd = vim.fn.getcwd()
    local neutral = vim.fn.tempname()
    fs.ensure_dir(neutral)
    vim.fn.chdir(neutral)
    local base = "/root/neoai_tools_dirtree_test"
    vim.fn.delete(base, "rf")
    local done = false
    executor.execute("run_command", {
      command = "mkdir -p " .. base .. "/sub/a " .. base .. "/sub/b; echo one > " .. base .. "/sub/a/one.txt; echo two > " .. base .. "/sub/b/two.txt",
      description = "建目录树",
    }, {}):then_(function()
      return executor.execute("list_files", { path = base, recursive = true, description = "列目录" }, {})
      ...
      t.matches("two%.txt", tostring(r), "应搜索到沙箱新建文件内容")
      t.false_(fs.exists(base), "真实磁盘不应出现该目录（未逃逸）")
      done = true
    end, function(e)
      t.true_(false, "不应失败: " .. tostring(e and e.message or e))
      done = true
    end)
    t.true_(vim.wait(20000, function() return done end), "沙箱目录一致性检查应完成")
    config_store.load(saved)
    vim.fn.chdir(prev_cwd)
    vim.fn.delete(neutral, "rf")
    vim.fn.delete(base, "rf")
  end)

  it("create_directory 产物对 list_files 可见，且沙箱路径不泄露到错误信息", function(t)
    local registry = require("NeoAI.tools.registry")
    registry.reset()
    local config_store = require("NeoAI.kernel.config_store")
    local saved = config_store.get_all()
    config_store.load({ tools = { approval = { mode = "async" }, sandbox = { mode = "dry_run", review = { enabled = true } } } })
    local executor = require("NeoAI.tools.executor")
    registry.register_many(require("NeoAI.tools.builtin.file_ops").get_tools())
    local sandbox = require("NeoAI.sandbox")
    sandbox.reset()
    ...
    executor.execute("create_directory", { filepath = dir, description = "建目录" }, {}):then_(function()
      ...
      local msg = tostring(e and e.message or e)
      t.matches("文件不存在", msg, "应返回文件不存在的清晰错误")
      t.true_(not msg:find("sessions", 1, true), "错误信息不应泄露沙箱暂存路径")
      done = true
    end)
    t.true_(vim.wait(10000, function() return done end), "检查应完成")
    config_store.load(saved)
    vim.fn.delete(dir, "rf")
  end)
