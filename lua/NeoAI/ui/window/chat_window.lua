...
local logger = {
  debug = function(msg, ...)
    ...
    print("[DEBUG] " .. msg, vim.log.levels.DEBUG)
  end,
  info = function(msg, ...)
    if select("#", ...) > 0 then
      msg = string.format(msg, ...)
    end
    print("[INFO] " .. msg, vim.log.levels.INFO)
  end,
  warn = function(msg, ...)
    if select("#", ...) > 0 then
      msg = string.format(msg, ...)
    end
    print("[WARN] " .. msg, vim.log.levels.WARN)
  end,
  error = function(msg, ...)
    if select("#", ...) > 0 then
      msg = string.format(msg, ...)
...
function M.render_messages()
  ...
  table.insert(
    content,
    string.format("会话: %s | 分支: %s", state.current_session_id or "无", state.current_branch_id or "无")
  )
...
function M.set_keymaps(keymap_manager)
  ...
  if keymap_manager then
    ...
    -- 检查 chat_keymaps 是否为空表或 nil
    if chat_keymaps and next(chat_keymaps) ~= nil then
      ...
      -- 如果 chat_keymaps 为空或 nil，使用配置或默认值
...
function M.add_message(role, content)
  ...
  -- 确保会话存在
  if not state.current_session_id or not state.current_branch_id then
    local session_created = M._ensure_session_exists()
    if not session_created then
      logger.error("无法创建会话，消息添加失败")
      return
    end
  end
...
function M._handle_send()
  ...
  local session_created = M._ensure_session_exists()
  if not session_created then
    vim.notify("无法创建会话，消息发送失败", vim.log.levels.ERROR)
    return
  end
...
function M._handle_ai_chunk(chunk)
  ...
  if chunk:match("^data: ") then
    ...
    if json_str and json_str ~= "[DONE]" then
      ...
      if ok and data and data.choices and data.choices[1] and data.choices[1].delta then
        local delta = data.choices[1].delta
        if delta.content then
          actual_content = delta.content
        elseif delta.reasoning_content then
          -- 显示实际的推理内容（截断以避免日志过长）
          local reasoning_text = tostring(delta.reasoning_content)
          if #reasoning_text > 50 then
            actual_content = "[思考内容] " .. string.sub(reasoning_text, 1, 50) .. "..."
          else
            actual_content = "[思考内容] " .. reasoning_text
          end
        else
          actual_content = "[其他数据]"
        end
      ...
      actual_content = "[DONE或无效数据]"
    end
  end

  -- 记录日志（只记录实际内容，不记录完整的SSE数据）
  logger.debug("收到AI响应数据块: " .. actual_content)
  ...
  -- 更新当前AI响应（使用实际内容）
  state.current_ai_response.content = state.current_ai_response.content .. actual_content
...
function M._load_messages()
  ...
  if #state.message_buffer > 0 then
    vim.notify(
      string.format("已加载 %d 条消息 (会话: %s, 分支: %s)", #state.message_buffer, session_id, branch_id),
      vim.log.levels.INFO
    )
  end
end
...
--- 确保会话存在（内部使用）
--- @return boolean 是否成功
function M._ensure_session_exists()
  -- 如果已经有会话ID，检查会话是否存在
  if state.current_session_id then
    -- 尝试获取会话管理器
    local ok, session_manager = pcall(require, "NeoAI.core.session.session_manager")
    if ok then
      local session = session_manager.get_session(state.current_session_id)
      if session then
        return true -- 会话已存在
      end
    end
  ...
  local ok, session_manager = pcall(require, "NeoAI.core.session.session_manager")
  if not ok then
    vim.notify("无法加载会话管理器", vim.log.levels.ERROR)
    return false
  end

  -- 创建默认会话
  local session_id = session_manager.create_session("聊天会话")
  if not session_id then
    vim.notify("无法创建会话", vim.log.levels.ERROR)
    return false
  end
  ...
  -- 通知用户
  vim.notify("✅ 已创建新会话: " .. session_id, vim.log.levels.INFO)
