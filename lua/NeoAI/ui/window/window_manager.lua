...
function M.initialize(config)
    ...
    -- 设置窗口模式
    if config and config.window_mode then
        state.window_mode = config.window_mode
    end
...
function M.create_window(window_type, options)
    ...
    -- 根据窗口模式创建窗口
    local window_info = window_mode_manager.create_window_by_mode(state.window_mode, merged_options)
...
function M.update_config(new_config)
    ...
    -- 更新窗口模式
    if new_config and new_config.window_mode then
        state.window_mode = new_config.window_mode
    end
end
...
function M.set_window_mode(mode)
    if not state.initialized then
        return
    end
