...
local function _sandboxed_argv(argv, opts)
  if not (opts and opts.prefix and #opts.prefix > 0) then
    return argv
  end
  local full = {}
  for _, v in ipairs(opts.prefix) do
    full[#full + 1] = v
  end
...
local function _run_command(command, opts)
  ...
  if not opts._egress_checked then
    ...
    if guard then
      ...
      return guard:then_(function()
        return _run_command(command, opts2)
      end)
    end
  end
  ...
  if max_wall > 0 and (timeout_ms < 0 or timeout_ms > max_wall) then
    timeout_ms = max_wall
  end
  ...
  local function append(chunks, data, which)
    if done or not data or #data == 0 then
      return
    end
...
--- 以 PTY 运行命令：轮询 /proc 检测“阻塞读终端 = 等待输入”，由判官子 agent 或用户手动注入。
--- 结果与 `_run_command` 同构（stdout/stderr），便于复用既有结果呈现。
--- @param command string
--- @param opts table
--- @return Deferred
local function _run_interactive(command, opts)
  opts = opts or {}
  ...
  if not opts._egress_checked then
    ...
    if guard then
      ...
      opts2._egress_checked = true
      return guard:then_(function()
        return _run_interactive(command, opts2)
      end)
    end
  end
  ...
  local full = {}
  for _, v in ipairs(opts.prefix or {}) do
    full[#full + 1] = v
  end
...
local function _with_status(status, out, errout)
  ...
  if errout and errout ~= "" then
    parts[#parts + 1] = errout
  end
  if #parts == 1 then
    parts[#parts + 1] = "（无输出）"
  end
...
local function _env_hint(text)
  ...
  if ok_cfg and type(cfg) == "table" and cfg.enabled ~= false then
    facade = true
  end
  if facade then
    return text
...
shell_tools.run_command = helpers.define_tool(
  ...
  function(args, on_success, on_error, ctx)
    ...
    local run_opts = {
      ...
      command = command,
      parent_agent = ctx and ctx.agent,
    }
    local icfg = require("NeoAI.kernel.config_store").get("tools.run_command.interactive") or {}
    local pty_svc = require("NeoAI.kernel.services").use("services.pty")
    local run_d
    if pty_svc and icfg.enabled and (icfg.engine or "auto") ~= "off" and not (ctx and ctx.sandbox_resident) then
      run_d = _run_interactive(command, run_opts)
    else
      run_d = _run_command(command, run_opts)
    end
    run_d:then_(function(result)
      -- 供沙箱门禁做权限不足检测（自动提权）：保留原始 {code,stdout,stderr}。
      if ctx then
        ctx.sandbox_last_result = result
      end
      ...
      return conceal.redact_async(result.stdout or ""):then_(function(out)
        return conceal.redact_async(result.stderr or ""):then_(function(errout)
          ...
          elseif result.oom then
            ...
            local where = (result.oom_level == "sandbox") and "沙箱资源域内存不足"
              or "容器/宿主内存不足（外层 cgroup OOM）"
            text = _with_status(
              "命令被终止（疑似内存超限 OOM）："
                .. where
                .. "，命令进程被内核杀死。"
            ...
            if cgroup_path then
              ...
              if ok_cg and cg and cg.events_snapshot then
                ...
                if snap.memory_peak then
                  parts[#parts + 1] = "memory.peak=" .. snap.memory_peak
                end
                if snap.memory_max then
                  parts[#parts + 1] = "memory.max=" .. snap.memory_max
                end
                if snap.memory_events then
                  parts[#parts + 1] = "memory.events={" .. snap.memory_events:gsub("%s+", ",") .. "}"
                end
                if snap.pids_events then
                  parts[#parts + 1] = "pids.events={" .. snap.pids_events:gsub("%s+", ",") .. "}"
                end
                if #parts > 0 then
                  diag = "\n资源域事件：" .. table.concat(parts, " ")
                end
              end
            end
            text = _with_status(
