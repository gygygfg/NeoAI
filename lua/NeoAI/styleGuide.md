...
├── tools/                      # 工具系统
│   ├── init.lua                # 工具模块入口
│   ├── approval_handler.lua    # 工具审批处理器（审批队列管理、对话框显示）
...
└── tests/                      # 测试
    ...
    ├── test_ai_core_part1.lua  # AI 核心测试 1：ai_engine, chat_service, response_retry
    ├── test_ai_core_part2.lua  # AI 核心测试 2：tool_orchestrator, request_adapter, request_builder, stream_processor
    ...
    ├── test_init_modules.lua   # 模块初始化测试：config_init, core_init, ai_init, ui_init, tools_init, utils_init, main_init
    ├── test_tools.lua          # 工具测试：tool_registry, tool_executor, tool_validator, tool_pack
    ├── test_utils.lua          # 工具库测试：utils_common, utils_logger, utils_json, utils_table_utils, utils_file_utils, async_worker
...
config_merger.process_config(config)      ← 一步完成：验证 → 合并 → 清理 → 初始化日志
    │
    ▼
state_manager.register_slice("config", { data = config })  ← 注册配置切片
state_manager.register_slice("app", { initialized = true }) ← 注册应用切片
    │
    ▼
core.initialize(config)                   ← 初始化核心模块
  ├── config_module.initialize(config)    ← 配置模块（keymap_manager + state，state 幂等）
  ├── ai_engine.initialize()              ← AI 引擎（注册 "ai_engine" 状态切片）
...
**关键变更**：
1. `config_merger.process_config()` 在 `state_manager.register_slice` **之前**调用，一步完成验证→合并→清理→初始化日志
2. `state_manager.register_slice("config", { data = config })` 在 `core.initialize()` **之前**调用，确保所有模块可通过 `state_manager.get_config()` 获取配置
...
6. AI 引擎初始化时注册 `"ai_engine"` 状态切片（tools, tool_definitions, active_generations 等），子模块通过 `state_manager` 读写共享状态
...
- `setup(user_config)` — 初始化所有模块、注册命令和快捷键
  - 初始化顺序：`config_merger.process_config` → `state_manager.register_slice` → `core.initialize` → `ui.initialize` → `tools.initialize` → 延迟注入工具 → 注册命令 → 注册快捷键 → `_auto_run_tests`
...
|- `get_config()` 通过 `state_manager.get_state("config", "data")` 获取配置，不再维护独立配置引用
...
|- 提供**状态切片**（slice）机制：模块注册自己的状态域，支持读写和变更监视
|- 提供**协程上下文**机制：协程安全的闭包上下文隔离（session_id、tools 等），协程内所有模块通过 `get_shared()` 直接读写共享表
|- 提供**事件触发辅助**：封装 Neovim 原生 User 事件，提供一致的触发接口
|- 核心 API：
|  - `register_slice(name, data)` — 注册状态切片
|  - `get_state(slice, key, default)` — 获取状态值（支持点号路径）
|  - `set_state(slice, key, value)` — 设置状态值并通知 watcher
|  - `watch(slice, key, callback)` — 监视状态变化
...
|- 已注册的状态切片：
|  - `"config"` — 完整配置数据
|  - `"app"` — 应用状态（initialized）
|  - `"ai_engine"` — AI 引擎状态（tools, tool_definitions, active_generations 等）
|  - `"tool_orchestrator"` — 工具编排器状态
...
|- 状态管理：跨模块共享状态（tools, tool_definitions, active_generations 等）通过 `state_manager` 的 `"ai_engine"` 状态切片管理
...
|- 构建工具结果消息
|- 状态管理：tool_definitions 和 first_request 存储在 `state_manager` 的 `"ai_engine"` 切片中
...
### `tools/approval_handler.lua` — 工具审批处理器
...
|- `process_queue()` — 处理审批队列（从队列头部取出一个待审批的工具）
...
|- 管理 UI 模块的事件监听
|- `register_listeners(state, callbacks)` — 注册 UI 事件监听（SESSION_CREATED, GENERATION_COMPLETED, STREAM_CHUNK 等）

### `ui/components/tool_approval.lua` — 工具审批悬浮窗组件

|- 用户按快捷键后打开一个模糊匹配选择器，选择要审批/拒绝的工具
|- 布局：标题栏 + 当前选中显示 + 过滤输入框 + 工具列表
|- 支持模糊匹配过滤
|- 支持快捷键选择
...
`tool_approval.lua` 组件提供工具审批功能：

- 用户按 `<C-a>` 打开审批悬浮窗
- 显示所有可用工具列表，支持模糊匹配过滤
- 选择工具后，该工具的所有调用都需要用户确认
- 审批队列由 `approval_handler.lua` 管理
...
2. **职责单一**：每个模块只负责一个领域的功能，AI 引擎已拆分为 ai_engine/http_utils/request_builder/stream_processor/generation_handler/session_manager 等子模块
3. **配置集中**：所有配置在 `default_config.lua` 中定义，`core/config/merger.lua` 的 `process_config()` 一步完成验证→合并→清理→初始化日志
4. **状态统一**：`core/config/state.lua` 集中管理共享状态，提供状态切片和协程上下文两种隔离机制
