...
function M.export_session(session_id, format)
  ...
  for branch_id, _ in pairs(session.branches) do
    ...
    if branch then
      ...
      -- 收集消息数据（获取所有消息，使用很大的限制值）
      local messages = message_manager.get_messages(branch_id, 1000000)
...
function M.import_session(data, format)
  ...
  -- 获取会话管理器
  local session_manager = require("NeoAI.core.session.session_manager")
  local session_branch_manager = session_manager.get_branch_manager()
  local session_message_manager = session_manager.get_message_manager()
  ...
  for branch_id, branch_data in pairs(import_data.branches or {}) do
    -- 创建分支
    local new_branch_id = session_branch_manager.create_branch(branch_data.parent_id, branch_data.name)

    -- 导入消息
    for msg_id, msg_data in pairs(import_data.messages or {}) do
      if msg_data.branch_id == branch_id then
        session_message_manager.add_message(new_branch_id, msg_data.role, msg_data.content, msg_data.metadata)
      end
    end
  end
