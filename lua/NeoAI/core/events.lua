...
M.TOOL_DISPLAY_RESIZED = "NeoAI:tool_display_resized" -- 工具调用悬浮窗大小/位置已变化
M.TOOL_APPROVED = "NeoAI:tool_approved" -- 工具已审批通过
M.TOOL_APPROVAL_CANCELLED = "NeoAI:tool_approval_cancelled" -- 工具审批已取消
M.TOOL_APPROVAL_CONFIG_CHANGED = "NeoAI:tool_approval_config_changed" -- 工具审批配置已变更
M.TOOL_APPROVAL_QUEUED = "NeoAI:tool_approval_queued" -- 工具已加入审批队列
M.TOOL_CALLS_READY = "NeoAI:tool_calls_ready" -- 流式工具调用 JSON 已完整（双触发机制）
M.TOOL_EXECUTION_ALL_COMPLETED = "NeoAI:tool_execution_all_completed" -- 本轮所有工具执行完毕，等待 AI 响应
...
-- ==================== 历史管理器事件 ====================
-- 使用位置: history_manager.lua, history_saver.lua
M.ROUND_ADDED = "NeoAI:round_added" -- 对话轮次已添加
M.ORPHANS_CLEANED = "NeoAI:orphans_cleaned" -- 孤儿会话已清理

-- ==================== 历史保存事件 ====================
-- 使用位置: chat_window.lua, history_saver.lua
M.HISTORY_SAVE_FINAL = "NeoAI:history_save_final" -- 保存最终 AI 回复（含 UI 构建的折叠文本）
