...
> kernel observation read a secret) and `⚠ 密钥：<tool> 使用了密钥（…）` (used — arguments carry a
> secret value/token/sensitive env name, or a use-type command such as `ssh -i`/`curl`/`gpg`
> references a secret file); when both hold it says "获取并使用了密钥". **Merely listing/viewing
...
sensitive installs (third-party repo/key changes) keep the high-risk label; neither auto-applies.
