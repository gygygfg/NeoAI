...
| `test_sandbox_service` | Long-lived services/mirrors/diagnostics: service start/logs/status/stop and registry cleanup, capturing service changes back into staging on stop, `long_lived` gate branch, pip/npm/maven mirror injection (incl. settings.xml), cgroup event snapshot and OOM detection |
