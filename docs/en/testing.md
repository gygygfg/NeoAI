...
| `test_sandbox_background` | Background-command facade: `&`/nohup/setsid detection (excluding `&&`/redirection/mid-command `&`), `run_command` background commands auto-promoted to a long-lived service and stoppable, non-background commands stay one-shot, fallback to old behavior when `auto_background=false` |
