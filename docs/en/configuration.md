...
sandbox = {
  ...
  -- Note: tools.run_command.interactive.enabled defaults to true, so run_command is forced onto the
  -- one-shot path (the resident command server's stdin is /dev/null, so it cannot be interactive) and
  -- this section does not apply by default; set tools.run_command.interactive.enabled=false to restore
  -- resident-sandbox semantics (background processes surviving across calls).
  resident = {
