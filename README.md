...
> **懒加载（默认，无需配置）**：`setup()` 只加载配置、引导内核并登记插件，同时注册全部
> `:NeoAI*` 命令与全局快捷键的**占位符**；首次调用时才**异步分两阶段**启动插件图——
> 阶段 1（UI/chat/status）就绪后立即打开界面，阶段 2（工具/沙箱/skills/mcp 等）在后台
> 分帧加载，期间编辑器不阻塞。只读访问器（`NeoAI.get_*_service()` / `get_statusline*()`）
> 不会触发启动，未就绪时返回 `nil`/空串。因此打开 Neovim 不会因 NeoAI 卡顿，也无需
> `lazy.nvim` 之类的命令懒加载。
...
require("NeoAI").setup({
  ...
  ai = {
    ...
    context_cache = {
      ...
      threshold_ratio = 0.8,             -- 达到该比例触发后台异步压缩（不阻塞、不弹窗）
      warn_ratio = 0.85,                 -- 接近上限时状态栏变色提示
      retain_ratio = 0.16,               -- 溢出恢复保留的最近历史比例（常规压缩折叠第一轮至倒数第二轮）
      retain_min_tokens = 4096,          -- 溢出恢复的尾部保留下限（token）
...
│   │   ├── compactor.lua      # 上下文压缩（后台异步 + 轮次范围 + 压缩覆盖层 + 辅助摘要）
...
│   │   ├── float_stream_window.lua # 复用流式悬浮窗（思考/参数/蒸馏共享）
